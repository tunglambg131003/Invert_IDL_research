...
def broyden_solver(f, x0, max_iter=50, tol=1e-6):
    """
    Vectorized Broyden's Good Method for root finding f(x) = 0.
    Finds the root for batch processing where x is shape (n, batch).
    """
    x = x0.clone()
    n, batch = x.shape
    
    # Initialize H (inverse Jacobian approximation) to Identity for each batch item
    # Shape: (batch, n, n). We transpose x internally to (batch, n) for batch matmuls
    ...
    for _ in range(max_iter):
        ...
        dx = (x_T - x_prev_T).unsqueeze(2)        # (batch, n, 1)
        df = (fx_T - fx_prev_T).unsqueeze(2)      # (batch, n, 1)
...
class ImplicitRootFinder(torch.autograd.Function):
    ...
    def backward(ctx, grad_X):
        ...
        with torch.no_grad():
            ...
            # Adjoint linear system: F(Y) = Y - grad_X - A^T (Phi * Y) = 0
            def f_adj(Y):
...
def main():
    ...
    optimizer = torch.optim.Adam(idl_flow.parameters(), lr=0.005)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=15000, eta_min=1e-5)
    
    num_steps = 15000
    
    print("Training Expressive Pure IDL Flow with Broyden's Method...")
    for idx_step in range(num_steps):
        X, _ = datasets.make_moons(n_samples=512, noise=0.05)
    ...
    plt.title("Generated Moons (Broyden Pure IDL)")
    plt.savefig('idl_spectral_moons.png')
